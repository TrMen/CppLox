...
using shared_stmt = std::shared_ptr<Statement>;
...
using FunctionStmt =
    StmtProduction<8, Token, std::vector<Token>,
                   std::vector<shared_stmt>>; // name params body

template <int id, typename... Types>
using StmtVisitable = VisitableImpl<StmtProduction<id, Types...>, Print,
                                    StmtExpr, Var, MalformedStmt, Block, IfStmt,
                                    EmptyStmt, WhileStmt, FunctionStmt>;
using StmtVisitor = Visitor<Print, StmtExpr, Var, MalformedStmt, Block, IfStmt,
                            EmptyStmt, WhileStmt, FunctionStmt>;
using StmtVisitableBase = Visitable<Print, StmtExpr, Var, MalformedStmt, Block,
                                    IfStmt, EmptyStmt, WhileStmt, FunctionStmt>;
...
std::ostream &operator<<(std::ostream &os, const shared_stmt &rhs);
std::ostream &operator<<(std::ostream &os, const std::vector<shared_stmt> &rhs);
...
template <typename Type, typename... arg_types>
stmt new_shared_stmt(arg_types &&...args)
{
